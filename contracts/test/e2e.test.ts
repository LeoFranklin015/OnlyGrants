...
describe("e2e", function test() {
    ...
    describe("Profile Management", () => {
        it("should allow a user to create a profile", async () => {
            await expect(minimalQF.connect(user).createProfile("User Profile"))
                .to.emit(minimalQF, "ProfileCreated")
                .withArgs(await user.getAddress())
        })

        it("should not allow creating a profile if one already exists", async () => {
            await expect(minimalQF.connect(user).createProfile("Another Profile"))
                .to.be.revertedWith("Profile already exists")
        })

        it("should allow adding a member to a profile", async () => {
            await minimalQF.connect(user).addProfileMember(user2)
            // You might need to add a getter function to check if a member was added
            // or emit an event to make this testable
        })

        it("should allow editing profile metadata", async () => {
            await minimalQF.connect(user).editProfileMetadata("Updated User Profile")
            // You might need to add a getter function to check if metadata was updated
            // or emit an event to make this testable
        })
    })
    ...
    describe("Round Management", () => {
        it("should allow creating a round", async () => {
            const matchingAmount = ethers.parseEther("1")
            const tx = await minimalQF.connect(user).createRound(
                matchingAmount,
                "Round Metadata",
                100n,
                treeDepths,
            ...
            const event = receipt!.logs.find(
                log => minimalQF.interface.parseLog(log as any)?.name === "RoundCreated"
            )
            roundId = minimalQF.interface.parseLog(event as any)!.args.roundId

            expect(roundId).to.not.be.undefined
        })

        it("should allow adding a member to a round", async () => {
            await minimalQF.connect(user).addRoundMember(roundId, user.getAddress())
            // You might need to add a getter function to check if a member was added
            // or emit an event to make this testable
        })

        it("should allow editing round metadata", async () => {
            await minimalQF.connect(user).editRoundMetadata(roundId, "Updated Round Metadata")
            // You might need to add a getter function to check if metadata was updated
            // or emit an event to make this testable
        })

        it("should allow applying for a round", async () => {
            const tx = await minimalQF.connect(user2).applyForRound(roundId, "Project Metadata", "0x")
            const receipt = await tx.wait()
            const event = receipt!.logs.find(
                log => minimalQF.interface.parseLog(log as any)?.name === "ProjectApplied"
            )
            expect(event).to.not.be.undefined
        })

        it("should allow donating to a project", async () => {
            const donationAmount = ethers.parseEther("0.1")
            await expect(minimalQF.connect(user2).donate(roundId, 0, donationAmount))
                .to.emit(minimalQF, "DonationMade")
                .withArgs(roundId, 0, user2.getAddress(), donationAmount)
        })

        it("should allow distributing funds for a completed round", async () => {
            // You might need to increase time to end the round
            await ethers.provider.send("evm_increaseTime", [101])
            await ethers.provider.send("evm_mine", [])

            await expect(minimalQF.connect(user).distributeFunds(roundId))
                .to.emit(minimalQF, "FundsDistributed")
                .withArgs(roundId)
        })

        it("should allow cancelling an active round", async () => {
            const newRoundId = roundId + 1n
            const matchingAmount = ethers.parseEther("1")
            await minimalQF.connect(user).createRound(
                matchingAmount,
                "New Round Metadata",
                100n,
                treeDepths,
            ...
            await expect(minimalQF.connect(user).cancelRound(newRoundId))
                .to.emit(minimalQF, "RoundCancelled")
                .withArgs(newRoundId)
        })
    })
})
